...
| Done (partly) | Supervise workers, handle malformed messages, and back off retries. | The [consumer](backend/src/worker/consume.rs) is no longer detached: if it stops, `main` exits so a supervisor restarts the process. Undecodable messages are logged and dropped. Throttled bulk tasks wait (up to 10 s) before requeue; storage-failed tasks wait 5 s. Still open: a readiness endpoint and a bounded retry count for repeated storage failures. See "Bulk-mode and deployment verification". |
...
| Done | Separate webhook delivery from verification retries. | The worker sends the webhook once, after the final result is stored and acked, with a 10 s timeout, non-2xx treated as failure, 3 attempts with growing delay, and an `x-reacher-task-id` idempotency header. Delivery failure is logged, not retried as a verification. (The SQS path still sends before storing.) |
| Done (partly) | Complete bulk limits and result accounting. | `max_bulk_emails` (default 10,000, `RCH__MAX_BULK_EMAILS`) caps `POST /v1/bulk`. Failed-task rows count as `unknown` in [progress](backend/src/http/v1/bulk/get_progress.rs), matching the CSV export. Still open: paginated CSV export docs and unbounded legacy v0 exports. |
...
| Done (single-check) | Make the deployment definition run this fork's code. | New [deploy/](deploy/README.md) Compose stack builds this fork's backend and adds a login-protected (HTTP basic auth) web page whose proxy injects the shared secret; the backend is not published and the page binds to 127.0.0.1 by default, with an optional Cloudflare tunnel profile. Still open: the RabbitMQ [Compose](rabbitmq/docker-compose.yaml) still uses upstream images, and there are no readiness checks. |
...
## Bulk-mode and deployment verification (2026-09-23, Docker Desktop)

- Backend unit tests: 16 passed (adds `webhook_retries_failed_status_and_sends_task_id`). SQLx offline cache regenerated with `cargo sqlx prepare --workspace` against PostgreSQL; all queries validated.
- Worker mode (2 requests/minute, `max_bulk_emails=5`): a malformed message published to the queue was logged ("Dropping malformed message") and the worker kept consuming. A 6-address job returned 400. A 4-address job with a webhook completed; while throttled the worker used 0% CPU (previously a tight requeue loop); the webhook receiver got exactly 4 deliveries with 4 distinct `x-reacher-task-id` values.
- A failed-task row (NULL result) counted as `unknown` in progress and exported with its error in CSV.
- `deploy/` stack with a test login: page and API returned 401 without a login or with a wrong password, 200 with the correct login; a check through the page returned a result; the backend had no host port.
